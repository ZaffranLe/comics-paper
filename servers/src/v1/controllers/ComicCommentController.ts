...
async function createComment(comicChapterId: number, authorId: number, content: string) {
    const comment = {
        comicChapterId,
        authorId,
        content,
...
async function deleteComment(commentId: string) {
    return await DatabaseBuilder(Tables.ComicComment).where({ id: commentId }).del();
}
