...
router.post(
    ...
    async (
        ...
        try {
            const { username, password, email, nickname, introduction } =
                req.body;
            ...
            if (nickname && !isValidNickname(nickname)) {
                ...
                return next(
                    new MiddlewareError(
                        Locale.HttpResponseMessage.InvalidNickname,
                        400
                    )
                );
            }
            ...
            if (!isValidPassword(password)) {
                return next(
                    new MiddlewareError(
                        Locale.HttpResponseMessage.InvalidPassword,
                        400
                    )
                );
            }
            ...
            if (await UserController.hasUserByUsername(username)) {
                return next(
                    new MiddlewareError(
                        Locale.HttpResponseMessage.UserAlreadyExists,
                        400
                    )
                );
            }
...
router.post(
    ...
    async (
        ...
        if (!user) {
            return next(
                new MiddlewareError(
                    Locale.HttpResponseMessage.UserNotFound,
                    400
                )
            );
        }
        ...
        if (!PasswordUtils.compare(password, user.password)) {
            return next(
                new MiddlewareError(
                    Locale.HttpResponseMessage.IncorrectPassword,
                    400
                )
            );
        }
...
router.get(
    ...
    async (
        ...
        if (!req["TokenRequest"]) {
            return next(
                new MiddlewareError(
                    Locale.HttpResponseMessage.NoTokenProvided,
                    400
                )
            );
        }
...
router.put(
    ...
    async (
        ...
        if (PasswordUtils.compare(newPassword, userRequest.password)) {
            return next(
                new MiddlewareError(
                    Locale.HttpResponseMessage.SamePassword,
                    400
                )
            );
        }
