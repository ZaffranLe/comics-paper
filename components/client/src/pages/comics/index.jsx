...
function Comics() {
  ...
  const fetchBookTags = async () => {
    try {
      ...
      const _tags = resp.data.map((_tag) => ({
        label: _tag.keyword,
        value: _tag.id,
      }));
      setTags(_tags);
  ...
  const handleSelectCategory = (selectedCategory) => {
    setSearchInfo({
      ...searchInfo,
      category:
        searchInfo.category === selectedCategory ? null : selectedCategory,
    });
  };
  ...
  const includedTagOptions = tags.filter(
    (_tag) => !searchInfo.notTags.includes(_tag.value),
  );
  const notIncludedTagOptions = tags.filter(
    (_tag) => !searchInfo.tags.includes(_tag.value),
  );
