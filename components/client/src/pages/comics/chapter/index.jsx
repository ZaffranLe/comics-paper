...
function ComicChapter() {
  ...
  const fetchComic = async (slug) => {
    console.info(`Trying to fetch the comic with slug: ${slug}`);
    try {
      const resp = await comicApi.getComicBySlug(slug);
      // console.log(resp.data.comic.chapters);
          ...
          {comic && chapter ? (
            <div className="mb-8">
              ...
              <div className="w-full text-center text-3xl font-bold my-4">
                {comic && comic.name} - {chapter && chapter.name}
              </div>
              <NavigateSection
                comic={comic}
                chapter={chapter}
              ...
              />
              {/* Content section */}
              {chapter && chapter.viewType === chapterViewTypes.IMAGE.VALUE && (
                <div>
              ...
              {chapter && chapter.viewType === chapterViewTypes.TEXT.VALUE && (
                <>
                  <div
                  ...
                  >
                    <div className="flex gap-2 pb-4">
                      ...
                      >
                        A
                        <FontAwesomeIcon icon="angle-double-down" fixedWidth />
                      </div>
                      <div
                        className={classNames(
                      ...
                      >
                        A <FontAwesomeIcon icon="angle-double-up" fixedWidth />
                      </div>
                      <div
...
function NavigateSection({ comic, chapter, chapterOptions }) {
  ...
  const handleChangeChapter = (selectedChapter) => {
    navigate(`../comics/${comic.slug}/chapter/${selectedChapter.value}`, {
      replace: false,
    });
  };
    ...
    <>
      ...
      <div className="my-4 flex items-center">
        ...
        <div className="flex flex-row gap-4 items-center">
          <div className="text-neutral-400 hidden md:block">
            Tips: Dùng Cmd+/Cmd- hoặc Ctrl+/Ctrl- để tăng giảm kích thước
          </div>
          {indexOfChapter !== 0 && (
            <button
