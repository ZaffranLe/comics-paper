...
async function createReview(comicId, userId, rating, content) {
    ...
    try {
        ...
        const comicInfo = await DatabaseBuilder(Tables.Comic)
            .where({ id: comicId })
            .first();
        const numberOfReviews = await DatabaseBuilder(Tables.ComicReview).where(
            { comicId }
        );
        const newRating =
            (comicInfo.likes * numberOfReviews.length + rating) /
            (numberOfReviews.length + 1);
        await transaction(Tables.ComicReview).insert(review);
        await transaction(Tables.Comic)
            .where({ id: comicId })
            .update({ likes: newRating });
