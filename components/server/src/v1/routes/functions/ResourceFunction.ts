...
async function handleUploadResource(
    ...
    Promise.all(
        uploadedFiles.map(async (file: Express.Multer.File) => {
            // Handle image
            console.log(
                `[resources] handling uploaded file ${file.originalname}`
            );
            ...
            // Hide path
            const { id, originalName, fileName, uploadedAt, uploader } =
                resource;
...
async function getBufferResource(req, res, next) {
    ...
    if (!resourceMetadata) {
        return next(
            new MiddlewareError(
                Locale.HttpResponseMessage.ResourceNotFound,
                404
            )
        );
    }
...
async function getMetadataResource(
    ...
    // Hide path
    const { id, originalName, fileName, uploadedAt, uploader, size, path } =
        resource;
