...
const CommentFunction = {
    createComment: async (req, res, next) => {
        try {
            ...
            if (!chapter) {
                return next(
                    new MiddlewareError(
                        Locale.HttpResponseMessage.ComicChapterNotFound,
                        404
                    )
                );
            }
            ...
            // Create comment
            const comment = await ComicCommentController.createComment(
                chapterId,
                user.id,
                content
            );
    ...
    getCommentByCommentId: async (req, res, next) => {
        try {
            ...
            if (!comment) {
                return next(
                    new MiddlewareError(
                        Locale.HttpResponseMessage.ComicCommentNotFound,
                        404
                    )
                );
            }
    ...
    getAllComments: async (req, res, next) => {
        try {
            ...
            // Get all comments
            const comments = await ComicCommentController.getCommentByChapterId(
                chapterId
            );
    ...
    getAllCommentsByUserId: async (req, res, next) => {
        try {
            ...
            const comments = await ComicCommentController.getCommentByUserId(
                userId
            );
            // Comment is not found
            if (!comments) {
