...
export const ComicFunction = {
    ...
    followComic: async (req, res, next) => {
        ...
        try {
            ...
            // Follow the comic
            const isFollowing = await ComicController.followComic(
                req.UserRequest.id,
                comicId
            );
            // Response
    ...
    getFollowState: async (req, res, next) => {
        ...
        try {
            ...
            const isFollowing = req.UserRequest
                ? await ComicController.getFollowState(
                      req.UserRequest.id,
                      comicId
                  )
                : false;
    ...
    getFollowingComics: async (req, res, next) => {
        try {
            const comics: ComicInterface[] =
                await ComicController.getFollowingComics();
            // Response
            res.json(comics);
        } catch (err) {
            return next(new MiddlewareError(err.message, 500));
    ...
    getComicByUser: async (req, res, next) => {
        try {
            ...
            const comics: ComicInterface[] =
                await ComicController.getComicByUser(userId);
            // Response
            res.json(comics);
        } catch (err) {
            return next(new MiddlewareError(err.message, 500));
    ...
    createNewComic: async (req, res, next) => {
        ...
        const { name, description, thumbnail, author, category, tags } =
            req.body;
        ...
        try {
            ...
            if (
                thumbnail &&
                !(await ResourceController.hasResource(thumbnail))
            ) {
                return next(
                    new MiddlewareError(
    ...
    createChapter: async (req, res, next) => {
        try {
            ...
            // Check permissions
            if (
                !(await user.hasPermission(PermissionEnum.COMIC_CHAPTER_CREATE))
            ) {
                return next(
                    new MiddlewareError(
                        Locale.HttpResponseMessage.Forbidden,
                        403
                    )
                );
            }
    ...
    deleteChapter: async (req, res, next) => {
        try {
            ...
            const generatedChapter = await ComicChapterController.deleteChapter(
                chapterId
            );
    ...
    getComicById: async (req, res, next) => {
        ...
        console.log(`getComicById; id = ${id}`);

        try {
            const comic: ComicInterface = await ComicController.getComicById(
                id
            );
            console.log(comic);
    ...
    getComicBySlug: async (req, res, next) => {
        ...
        try {
            const comic: ComicInterface = await ComicController.getComicBySlug(
                slug
            );
            // not found
            if (!comic) {
    ...
    countView: async (req, res, next) => {
        try {
            res.json({
                totalView: await ComicController.getViewNumberFromAllComic(),
            });
        } catch (err) {
            return next(new MiddlewareError(err.message, 500));
        }
