...
const ReviewFunction = {
    ...
    getReviewById: async (req, res, next) => {
        try {
            ...
            const currentReview = await ReviewController.getReviewById(
                reviewId
            );
            if (!currentReview) {
                return next(
                    new MiddlewareError(
                        Locale.HttpResponseMessage.ReviewNotFound,
                        404
                    )
                );
            }
    ...
    getReviewsByComicId: async (req, res, next) => {
        try {
            ...
            const currentReviews = await ReviewController.getReviewsByComicId(
                comicId
            );
    ...
    getReviewsByUserId: async (req, res, next) => {
        try {
            ...
            const currentReviews = await ReviewController.getReviewsByUserId(
                userId
            );
